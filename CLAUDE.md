...
**P&L exits are Fixed %-only:** `signal_profit`/`signal_loss` fire when `current_pnl` (computed from `pnl_entry_price`) crosses `±pnlTarget` — exact V50 behavior. *(V51B briefly added an ATR-Based alternative — `exitMode`, `stopATRMult`/`targetATRMult`, frozen `pnl_stop_price`/`pnl_target_price` — plus an optional Time Stop. Time Stop was removed in V51H; ATR-Based Exits were removed in V51I. Nothing from V51B remains.)*
...
14. **Time Stop removal (V51H):** confirm there is no `Enable Time Stop`/`Time Stop Bars` input and no `TIME STOP` alert or exit label ever fires
15. **ATR-Based Exits removal (V51I):** confirm there is no `Exit Mode` input (P&L Exit Signals is Fixed %-only), and PROFIT/LOSS behave identically to V50

---

## Version Naming

The script file is `SuperLazyTrade.pine` (version-agnostic name). The current version is **V51I**. Version identifiers follow major=significant feature additions, minor letter=incremental fixes. The full changelog is documented in the block comments at the top of the file.
...
- **V51B** — ATR-Based Exits + Time Stop (Phase 2 of the V51 design-review series): `exitMode` input (default `"Fixed %"`, preserves V50) adds an ATR-Based alternative with stop/target frozen at entry (`stopATRMult`/`targetATRMult` × intraday ATR); new `useTimeStop`/`timeStopBars` (default off) forces an exit after N bars with the actual P&L sign recorded as win/loss; new `TIME STOP` alert *(Time Stop removed in V51H; ATR-Based Exits removed in V51I — nothing from V51B remains)*
...
- **V51H** — Time Stop Removed: no observed value from forcing an exit after N bars regardless of P&L, so the Time Stop half of V51B was removed — `useTimeStop`/`timeStopBars` inputs, `pnl_entry_bar` (and its assignment at all 8 P&L entry/re-arm branches), `time_stop_due`/`signal_time_stop`, the TIME STOP exit label, its success-rate capture, and its `alertcondition`. ATR-Based Exits (the other half of V51B) were untouched at this point; alerts reduced from 5 to 4.
- **V51I** — ATR-Based Exits Removed: P&L Exit Signals reduced to Fixed %-only — `exitMode` input and its "ATR-Based" option, `stopATRMult`/`targetATRMult` inputs, `pnl_stop_price`/`pnl_target_price` (and their freeze assignment at all 8 P&L entry/re-arm branches), the ATR-Based branch of target/stop hit detection, and the ATR-Based branch of PROFIT/LOSS label text. `target_hit`/`stop_hit` and PROFIT/LOSS labels now always compute from `current_pnl` vs `pnlTarget`, identical to V50. Together with V51H, nothing from V51B remains.
