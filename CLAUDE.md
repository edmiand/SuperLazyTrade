...
11. **P&L tracking** — Two independent systems: (a) live dashboard P&L using `pnl_entry_price`/`pnl_direction` with Last Signal or First Signal reset logic; (b) signal-to-signal `signal_pnl` history shown on labels. PROFIT/LOSS exit triggers are `exitMode`-dependent (Fixed % or ATR-Based, V51B) — see [ATR-Based Exits](#key-design-decisions).
12. **Success rate tracking** — Rolling arrays (`buy_exit_results`/`sell_exit_results` for PROFIT/LOSS outcomes, `buy_reversal_results`/`sell_reversal_results` for positions closed by an opposite signal, V51C) capped at `maxSignalsToTrack`; updated on exits first (to prevent double-count), then on new signals.
...
> **Historical note:** An earlier "Gate 5 (Breakout Bonus +15)" existed before V48B and was retired — moved to Component 5C. A later Gate 5 (HTF Counter-Trend), added in V49B and extended with a Hard Veto mode in V51E, was itself removed in V51G after testing showed no observable difference in signal behavior. There is no Gate 5 in the current file.
...
**ATR-Based Exits (V51B):** `exitMode` (`"Fixed %"` default, or `"ATR-Based"`) controls how `signal_profit`/`signal_loss` trigger. Fixed % checks `current_pnl` against `±pnlTarget` (exact V50 behavior). ATR-Based checks `close` against `pnl_stop_price`/`pnl_target_price` — frozen levels computed **once at entry** as `entry ∓ stopATRMult × atr` / `entry ± targetATRMult × atr` (intraday `atr`, not the daily `atr_14` used by the ATR Fuel gauge). The freeze is inlined at each of the 8 LAST/FIRST-signal-mode entry branches in `UPDATE P&L EXIT TRACKING`, only where the entry price actually changes; the First-Signal-mode same-direction-continuation branches (entry price intentionally kept) reset only `pnl_exit_fired`, leaving stop/target untouched so they never drift. PROFIT/LOSS labels show the actual realized `current_pnl` in ATR-Based mode instead of the static target text. *(V51B originally paired this with an optional Time Stop — `useTimeStop`/`timeStopBars`/`pnl_entry_bar`/`signal_time_stop` — that forced an exit after N bars; it was removed in V51H as low-value. ATR-Based Exits themselves are unaffected.)*

**Split Exit vs Reversal win-rate tracking (V51C):** `buy_results`/`sell_results` used to mix two incompatible outcome definitions — an entry closed by PROFIT/LOSS vs. an entry closed by the next opposite-direction signal — in one array, making the win rate unable to validate parameter changes (its stated purpose). Now split into `*_exit_results` (target/stop only) and `*_reversal_results` (closed by opposite signal, captured in the same "Process BUY/SELL success tracking" branch that always existed — it's a reversal by construction since `b_fired`/`s_fired` block same-direction repeats). PROFIT/LOSS exits are still captured *before* new-signal processing on the same bar to prevent double-counting (`resultCaptured_buy`/`resultCaptured_sell`). Each of the 4 arrays is an independent rolling `array<bool>` capped at `maxSignalsToTrack` via FIFO (`array.shift` on overflow). `calc_win_rate(arr, enabled) => [wins, total, rate]` computes stats for all 4 from one shared read-only function.

**Dashboard row budget:** `DASHBOARD_MAX_ROWS = 28` (rows 0–27; raised from 25 → 27 in V51C, then 27 → 28 in V51F — see below). With all options enabled + 4 active gates, the dashboard now uses one fewer row than before V51G (the HTF row was removed along with the gate). The gate detail loop is the last section; it has an explicit `if row >= DASHBOARD_MAX_ROWS: break` guard because it is the only variable-length section (all other rows are fixed-count by design). V51C's BUY/SELL Reversal rows and V51F's "Bars From Flip" row are both gated behind (or only present within) `extDash` (Extended Metrics) — with it off, Success Rate Tracking still occupies exactly the 3 rows (separator + BUY Exits + SELL Exits) V50's BUY/SELL Win Rate did, and there's no Extended Metrics section at all. `DASHBOARD_MAX_ROWS` was not lowered after V51G's HTF-row removal — the existing budget just has one more row of headroom than the worst case requires.
...
8. **Dashboard row count:** with Success Rate Tracking + all 4 gates active + Extended Metrics ON, confirm no runtime error (row overflow guard working; `DASHBOARD_MAX_ROWS = 27` as of V51C)
...
11. **Split win rate (V51C):** with `Enable Success Rate Tracking` on, confirm the dashboard shows `BUY Exits`/`SELL Exits` always, and `BUY Reversal`/`SELL Reversal` only when `Extended Metrics` is also on; confirm a PROFIT/LOSS exit increments only the matching `*_exit_results` count and a signal-closed-by-opposite-signal increments only `*_reversal_results` (never both for the same entry)
12. **Entry freshness (V51F):** in Score mode, set `Max Bars From Flip = 5` → confirm no BUY/SELL fires on bar 6+ after a flip (watch the "Bars From Flip" Extended Metrics row to confirm the count itself resets to 0 on each flip bar); confirm Trend mode is unaffected by the same setting; confirm worst case (Success Rate + Extended Metrics + all 4 gates on) still shows no dashboard row overflow with `DASHBOARD_MAX_ROWS = 28`
13. **HTF removal (V51G):** confirm there is no "📡 HTF Trend Filter" input group, no HTF row on the dashboard, and `Enable Risk Gates` toggling only ever affects Squeeze/Stretch/ATR Fuel/Liquidity
14. **Time Stop removal (V51H):** confirm there is no `Enable Time Stop`/`Time Stop Bars` input, no `TIME STOP` alert or exit label ever fires, and ATR-Based Exits (`Exit Mode = ATR-Based`) still behave exactly as before (frozen stop/target, correct realized % on PROFIT/LOSS labels)

---

## Version Naming

The script file is `SuperLazyTrade.pine` (version-agnostic name). The current version is **V51H**. Version identifiers follow major=significant feature additions, minor letter=incremental fixes. The full changelog is documented in the block comments at the top of the file.
...
- **V49B** — HTF counter-trend filter added as new Gate 5 (unrelated to retired V48B Gate 5); gate count back to 5; `ema_stretch` and `trend_stretch` unified via intraday ATR *(this Gate 5 was itself removed in V51G)*
...
- **V51B** — ATR-Based Exits + Time Stop (Phase 2 of the V51 design-review series): `exitMode` input (default `"Fixed %"`, preserves V50) adds an ATR-Based alternative with stop/target frozen at entry (`stopATRMult`/`targetATRMult` × intraday ATR); new `useTimeStop`/`timeStopBars` (default off) forces an exit after N bars with the actual P&L sign recorded as win/loss; new `TIME STOP` alert *(the Time Stop half was removed in V51H; ATR-Based Exits remain)*
...
- **V51E** — HTF Counter-Trend: Penalty → Optional Veto (Phase 5 of the V51 design-review series): new `htfMode` input (default `"Penalty (-20)"`, preserves V50 Gate 5 behavior) adds a `"Hard Veto"` alternative that blocks `signal_buy`/`signal_sell` outright on a counter-HTF signal, independent of `gateOn` and score; exits unaffected; dashboard HTF row shows `⛔ VETO` when blocking *(this entire feature, including its Gate 5 base, was removed in V51G)*
...
- **V51G** — HTF Counter-Trend Filter Removed: user testing found no observable difference in signal behavior from Gate 5 (HTF Counter-Trend, V49B/V51E), so it was removed entirely — the `📡 HTF Trend Filter` input group, `htf_ema9`/`htf_ema20`/`htf_is_bull`/`htf_is_bear`, `gate5_htf`/`gate5_penalty`/`HTF_PENALTY`, `htf_veto_block`, and the dashboard HTF row. Risk gates reduced from 5 back to 4 (Squeeze, Stretch, ATR Fuel, Liquidity); all other V51 phases (RVOL, ATR exits, split win-rate, entry freshness) unaffected.
- **V51H** — Time Stop Removed: no observed value from forcing an exit after N bars regardless of P&L, so the Time Stop half of V51B was removed — `useTimeStop`/`timeStopBars` inputs, `pnl_entry_bar` (and its assignment at all 8 P&L entry/re-arm branches), `time_stop_due`/`signal_time_stop`, the TIME STOP exit label, its success-rate capture, and its `alertcondition`. ATR-Based Exits (the other half of V51B) are untouched; alerts reduced from 5 to 4.
