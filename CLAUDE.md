...
**Split Exit vs Reversal win-rate tracking:** `buy_results`/`sell_results` would otherwise mix two incompatible outcome definitions — an entry closed by PROFIT/LOSS vs. an entry closed by the next opposite-direction signal — in one array, making the win rate unable to validate parameter changes (its stated purpose). Instead they're split into `*_exit_results` (target/stop only) and `*_reversal_results` (closed by opposite signal, captured in the same "Process BUY/SELL success tracking" branch — it's a reversal by construction since `b_fired`/`s_fired` block same-direction repeats). PROFIT/LOSS exits are captured *before* new-signal processing on the same bar to prevent double-counting (`resultCaptured_buy`/`resultCaptured_sell`). Each of the 4 arrays is an independent rolling `array<bool>` capped at `maxSignalsToTrack` via FIFO (`array.shift` on overflow). `calc_win_rate(arr, enabled) => [wins, total, rate]` computes stats for all 4 from one shared read-only function.

**Dashboard row budget:** `DASHBOARD_MAX_ROWS = 28` (rows 0–27). With all options enabled + 4 active gates, the dashboard has one row of headroom below that cap. The gate detail loop is the last section; it has an explicit `if row >= DASHBOARD_MAX_ROWS: break` guard because it is the only variable-length section (all other rows are fixed-count by design). The BUY/SELL Reversal rows and the "Bars From Flip" row are both gated behind (or only present within) `extDash` (Extended Metrics) — with it off, Success Rate Tracking occupies exactly 3 rows (separator + BUY Exits + SELL Exits), and there's no Extended Metrics section at all.
