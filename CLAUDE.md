...
- **V51D** — Time-of-Day Filters (Phase 4 of the V51 design-review series): added, then removed as low-value. Originally added an `"⏰ Time-of-Day Filters"` input group (open-minutes block, lunch-session block, no-new-entries-after cutoff) gating `signal_buy`/`signal_sell` only; fully reverted — no `grp_time` inputs, `time_filters_ok` flag, or dashboard `⏰` label remain in the current file.
