...
5. **Core indicator calculations** — EMAs (9, `emaSlow`=user-choice 20 or 30, 20 fixed for scoring, 50), VWAP, ATR(14), RSI(14), ADX(14,14), MACD(12,26,9), relative volume (`rvolMode`: Time-of-Day default, Rolling 20-bar SMA fallback/alternative — see [Time-of-Day Relative Volume](#key-design-decisions)), HTF EMA9/20 via `request.security`
...
10. **Signal generation** — Signals are non-repainting; `barstate.isconfirmed` guards all flip conditions. HTF gate display and dashboard can flicker intrabar (live HTF bar values update continuously). In "Score" signal-timing mode, alternating BUY/SELL is enforced via `b_fired`/`s_fired` flags; in "Trend" mode, signals fire directly off `trendUp`/`trendDown` and never check these flags — alternation there falls out of the flip-bar semantics instead. Flags reset when anchor direction changes (`is_bull` vs `is_bull[1]`), not just on flip bars. RTH filter via `time(timeframe.period, "0930-1600:23456")`. Three additional Time-of-Day Filters (V51D, all default off) can further suppress `signal_buy`/`signal_sell` only — see [Time-of-Day Filters](#key-design-decisions). Score-mode signals can also be constrained to fire only within `maxBarsFromFlip` bars of the anchor flip (V51F, default 0 = disabled); Trend mode is unaffected.
11. **P&L tracking** — Two independent systems: (a) live dashboard P&L using `pnl_entry_price`/`pnl_direction` with Last Signal or First Signal reset logic; (b) signal-to-signal `signal_pnl` history shown on labels. PROFIT/LOSS exit triggers are `exitMode`-dependent (Fixed % or ATR-Based, V51B) plus an optional Time Stop — see [ATR-Based Exits & Time Stop](#key-design-decisions).
12. **Success rate tracking** — Rolling arrays (`buy_exit_results`/`sell_exit_results` for PROFIT/LOSS/TIME STOP outcomes, `buy_reversal_results`/`sell_reversal_results` for positions closed by an opposite signal, V51C) capped at `maxSignalsToTrack`; updated on exits first (to prevent double-count), then on new signals.
...
15. **Alerts** — Five `alertcondition` calls with plaintext messages: `"BUY"`, `"SELL"`, `"PROFIT"`, `"LOSS"`, `"TIME STOP"` (V51B).
...
- RVOL denominator is `rvolMode`-dependent (V51A) — point tiers themselves never changed. See [Time-of-Day Relative Volume](#key-design-decisions).
...
| 5 — HTF Counter-Trend | HTF EMA9/20 direction opposes signal direction (when `htfEnabled`) | −20 (Penalty mode) or hard block (Veto mode) | `HTF_PENALTY` |
...
**HTF Counter-Trend enforcement strength (V51E):** `htfMode` (`"Penalty (-20)"` default, or `"Hard Veto"`) controls how `gate5_htf` is enforced. Penalty mode is the original Gate 5 behavior — a score penalty, only actually subtracted when `gateOn = true`, so a high-scoring counter-HTF setup can still fire. Hard Veto adds an independent, unconditional block: `htf_veto_block = htfMode == "Hard Veto" and gate5_htf` is AND'd into `signal_buy`/`signal_sell` regardless of `gateOn` or score. See [HTF Counter-Trend: Penalty vs Veto](#key-design-decisions).
...
**Time-of-Day Filters (V51D):** Three independent, all-default-off filters — `useBlockOpen`+`blockOpenMinutes` (first N minutes after 9:30 ET), `blockLunch`+`lunchStart`/`lunchEnd` (HHMM ET window), `useNoNewEntriesAfter`+`noNewEntriesAfter` (HHMM ET cutoff) — reuse the identical `time(timeframe.period, "HHMM-HHMM:23456")` session-string mechanism as `useSessionFilter`, so the **same ET-hardcoded caveat applies**: not appropriate for crypto / 24h futures without user awareness. They apply *only* to `signal_buy`/`signal_sell` via a combined `time_filters_ok` flag — `signal_profit`/`signal_loss`/`signal_time_stop` are computed earlier in the script and are never touched, so existing positions can always exit. The late-cutoff check uses a `"<cutoff>-2359:23456"` session (not `"0930-<cutoff>"` negated) specifically so it doesn't also match the pre-market "not yet 9:30" case. These are informational/hard-block flags, not scored gates — no point penalty, not counted in `active_gate_count`. Surfaced on the dashboard's always-visible `Gates`/`Risks` row (e.g. `2 ACTIVE ⏰ LUNCH BLOCK`) since that directly answers "why didn't a signal fire," and also in the Extended-Metrics-gated Gate/Risk Details breakdown.

**HTF Counter-Trend: Penalty vs Veto (V51E):** `htfMode` defaults to `"Penalty (-20)"`, the exact V50 Gate 5 behavior — a score penalty that only actually applies when `gateOn = true`, meaning a high-scoring counter-HTF setup can still fire. `"Hard Veto"` adds an unconditional block on top: `htf_veto_block = htfMode == "Hard Veto" and gate5_htf`, AND'd into `signal_buy`/`signal_sell` in both Trend and Score modes, independent of `gateOn` and independent of score. `gate5_htf`/`gate5_penalty`/`gate_message` computation is otherwise untouched — Hard Veto doesn't change how the penalty itself is computed, it just adds a second, independent block. Exits are unaffected. Dashboard HTF row shows `⛔ VETO` (not `⚠️ COUNTER`) specifically when Hard Veto is the reason a signal didn't fire.

**Entry freshness constraint (V51F):** `maxBarsFromFlip` (0-50, default 0 = disabled) constrains Score-mode entries to fire only within N bars of the most recent anchor flip, via `entry_is_fresh = maxBarsFromFlip <= 0 or bars_since_flip <= maxBarsFromFlip` AND'd into the Score-mode branch only. `bars_since_flip` (`var int`) resets to 0 on the flip bar itself and increments every bar after, tracked alongside the existing `trend_start_price` reset so both stay in sync with the same flip event. Trend mode is deliberately unaffected — it already fires directly on the flip bar by construction, so there's nothing to constrain. This is independent of (and complements) the Stretch gate, which only actually penalizes extension when `gateOn = true`; `maxBarsFromFlip` works regardless of gate mode. Calibrate using the Extended Metrics "Bars From Flip" row.
...
**ATR-Based Exits & Time Stop (V51B):** `exitMode` (`"Fixed %"` default, or `"ATR-Based"`) controls how `signal_profit`/`signal_loss` trigger. Fixed % checks `current_pnl` against `±pnlTarget` (exact V50 behavior). ATR-Based checks `close` against `pnl_stop_price`/`pnl_target_price` — frozen levels computed **once at entry** as `entry ∓ stopATRMult × atr` / `entry ± targetATRMult × atr` (intraday `atr`, not the daily `atr_14` used by the ATR Fuel gauge). The freeze is inlined at each of the 8 LAST/FIRST-signal-mode entry branches in `UPDATE P&L EXIT TRACKING`, only where the entry price actually changes; the First-Signal-mode same-direction-continuation branches (entry price intentionally kept) reset only `pnl_exit_fired` and the time-stop clock (`pnl_entry_bar`), leaving stop/target untouched so they never drift. An independent `useTimeStop`/`timeStopBars` pair (default off) forces a `signal_time_stop` exit if neither target nor stop fires within N bars of entry; its win/loss outcome is the actual sign of `current_pnl` at that bar, not automatically a loss. PROFIT/LOSS labels show the actual realized `current_pnl` in ATR-Based mode instead of the static target text.

**Split Exit vs Reversal win-rate tracking (V51C):** `buy_results`/`sell_results` used to mix two incompatible outcome definitions — an entry closed by PROFIT/LOSS/TIME STOP vs. an entry closed by the next opposite-direction signal — in one array, making the win rate unable to validate parameter changes (its stated purpose). Now split into `*_exit_results` (target/stop/time-stop only) and `*_reversal_results` (closed by opposite signal, captured in the same "Process BUY/SELL success tracking" branch that always existed — it's a reversal by construction since `b_fired`/`s_fired` block same-direction repeats). PROFIT/LOSS/TIME STOP exits are still captured *before* new-signal processing on the same bar to prevent double-counting (`resultCaptured_buy`/`resultCaptured_sell`). Each of the 4 arrays is an independent rolling `array<bool>` capped at `maxSignalsToTrack` via FIFO (`array.shift` on overflow). `calc_win_rate(arr, enabled) => [wins, total, rate]` computes stats for all 4 from one shared read-only function.

**Dashboard row budget:** `DASHBOARD_MAX_ROWS = 28` (rows 0–27; raised from 25 → 27 in V51C, then 27 → 28 in V51F — see below). With all options enabled + 5 active gates, the dashboard uses up to ~27 rows. The gate detail loop is the last section; it has an explicit `if row >= DASHBOARD_MAX_ROWS: break` guard because it is the only variable-length section (all other rows are fixed-count by design). V51C's BUY/SELL Reversal rows and V51F's "Bars From Flip" row are both gated behind (or only present within) `extDash` (Extended Metrics) — with it off, Success Rate Tracking still occupies exactly the 3 rows (separator + BUY Exits + SELL Exits) V50's BUY/SELL Win Rate did, and there's no Extended Metrics section at all. The worst case (Extended Metrics on) adds the 2 reversal rows + 1 "Bars From Flip" row, pushing the gate-details loop's start from row 19 (V50) to row 22 (V51F) — `DASHBOARD_MAX_ROWS` was raised by the matching +3 total (25→28) specifically to preserve the gate-details loop's row budget exactly (6 available row slots before the break guard, unchanged since V50).
...
8. **Dashboard row count:** with HTF enabled + Success Rate Tracking + all 5 gates active + Extended Metrics ON, confirm no runtime error (row overflow guard working; `DASHBOARD_MAX_ROWS = 27` as of V51C)
9. **RVOL mode (V51A):** toggle `Relative Volume Mode` between `Rolling 20-bar` and `Time-of-Day` on the same chart → confirm the Volume row's RVOL multiplier and mode label (`TOD` / `20-bar`) both change, and that a chart with less than `RVOL Lookback Sessions` of history shows `20-bar*` (fallback) instead of `na` or a stale value
10. **Exit Mode (V51B):** set `Exit Mode = ATR-Based` → confirm `pnl_stop_price`/`pnl_target_price` (visible via PROFIT/LOSS label values) stay constant for the life of one position and don't drift bar-to-bar; confirm labels show the actual realized % (e.g. `PROFIT +1.84%`), not `stopATRMult`/`targetATRMult`
11. **Time Stop (V51B):** set `Enable Time Stop = true` with a small `Time Stop Bars` (e.g. 5) → confirm a `TIME STOP` label fires exactly 5 bars after entry when neither target nor stop was hit, and that its win/loss is recorded correctly (positive P&L → win, negative → loss) in the success-rate arrays
12. **Split win rate (V51C):** with `Enable Success Rate Tracking` on, confirm the dashboard shows `BUY Exits`/`SELL Exits` always, and `BUY Reversal`/`SELL Reversal` only when `Extended Metrics` is also on; confirm a PROFIT/LOSS/TIME STOP exit increments only the matching `*_exit_results` count and a signal-closed-by-opposite-signal increments only `*_reversal_results` (never both for the same entry)
13. **Time-of-Day Filters (V51D):** enable each of the three filters one at a time (`Block First N Minutes After Open`, `Block Lunch Session`, `Block New Entries After Time`) → confirm new BUY/SELL signals are suppressed during that window and the `Gates`/`Risks` dashboard row shows the matching `⏰` label; confirm PROFIT/LOSS/TIME STOP exits still fire normally during a blocked window (a position opened before the block started should still be able to close)
14. **HTF Hard Veto (V51E):** set `HTF Counter-Trend Mode = Hard Veto` with `Enable Risk Gates = false` (default) → confirm a counter-HTF signal is still blocked (proves the veto is independent of `gateOn`); confirm the dashboard HTF row shows `⛔ VETO`; confirm a PROFIT/LOSS/TIME STOP exit on an existing position still fires while the veto is active
15. **Entry freshness (V51F):** in Score mode, set `Max Bars From Flip = 5` → confirm no BUY/SELL fires on bar 6+ after a flip (watch the "Bars From Flip" Extended Metrics row to confirm the count itself resets to 0 on each flip bar); confirm Trend mode is unaffected by the same setting; confirm worst case (HTF + Success Rate + Extended Metrics + all 5 gates on) still shows no dashboard row overflow with `DASHBOARD_MAX_ROWS = 28`

---

## Version Naming

The script file is `SuperLazyTrade.pine` (version-agnostic name). The current version is **V51F**. Version identifiers follow major=significant feature additions, minor letter=incremental fixes. The full changelog is documented in the block comments at the top of the file.
...
- **V51A** — Time-of-Day Relative Volume (Phase 1 of the V51 design-review series): `rvolMode` input (default `"Time-of-Day"`) replaces the trailing-20-bar RVOL denominator with a same-bar-slot average across prior sessions, removing Component 3's time-of-day scoring bias; falls back to the legacy Rolling 20-bar method automatically when session history is insufficient
- **V51B** — ATR-Based Exits + Time Stop (Phase 2 of the V51 design-review series): `exitMode` input (default `"Fixed %"`, preserves V50) adds an ATR-Based alternative with stop/target frozen at entry (`stopATRMult`/`targetATRMult` × intraday ATR); new `useTimeStop`/`timeStopBars` (default off) forces an exit after N bars with the actual P&L sign recorded as win/loss; new `TIME STOP` alert
- **V51C** — Split Win-Rate Measurement (Phase 3 of the V51 design-review series): success-rate arrays split into `*_exit_results` (target/stop/time-stop) and `*_reversal_results` (closed by opposite signal) per direction, so the win rate stops mixing two incompatible outcome definitions; dashboard shows both (Reversal rows gated behind Extended Metrics); `DASHBOARD_MAX_ROWS` raised 25 → 27 to fit the new rows while preserving V50's gate-details row budget exactly
- **V51D** — Time-of-Day Filters (Phase 4 of the V51 design-review series): new `"⏰ Time-of-Day Filters"` input group (all default off) adds an open-minutes block, a lunch-session block, and a no-new-entries-after cutoff, all applying only to `signal_buy`/`signal_sell` (never to exits); reuses the RTH filter's ET session-string mechanism and carries the same crypto/24h-futures caveat; surfaced on the dashboard's `Gates`/`Risks` row
- **V51E** — HTF Counter-Trend: Penalty → Optional Veto (Phase 5 of the V51 design-review series): new `htfMode` input (default `"Penalty (-20)"`, preserves V50 Gate 5 behavior) adds a `"Hard Veto"` alternative that blocks `signal_buy`/`signal_sell` outright on a counter-HTF signal, independent of `gateOn` and score; exits unaffected; dashboard HTF row shows `⛔ VETO` when blocking
- **V51F** — Entry Freshness Constraint (Phase 6 of the V51 design-review series, final phase): new `maxBarsFromFlip` input (0-50, default 0 = disabled, preserves V50) constrains Score-mode signals to fire only within N bars of the most recent anchor flip; Trend mode unaffected (fires on the flip bar by construction); new "Bars From Flip" Extended Metrics row; `DASHBOARD_MAX_ROWS` raised 27 → 28 for the new row, preserving the gate-details loop's row budget exactly
